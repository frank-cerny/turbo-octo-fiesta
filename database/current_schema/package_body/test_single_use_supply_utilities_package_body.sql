  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DEV_WS"."TEST_SINGLE_USE_SUPPLY_UTILITIES" 
as
    ...
    begin
        ...
        SELECT count(id) INTO rowCount from BSA_SINGLE_USE_SUPPLY where name = 'single use supply 22222';
    ...
    begin
        ...
        -- Act
        sus.bsa_proc_split_single_use_supply_among_projects('' || projectId || ':' || projectId1, 'single use supply 22222', 'item description', CURRENT_DATE, 'N', 5.00, 1, NULL);
    ...
    begin
        ...
        -- Act
        sus.bsa_proc_split_single_use_supply_among_projects('' || projectId || ':' || projectId1, 'single use supply 22222', 'item description', CURRENT_DATE, 'N', 5.01, 1, NULL);
    ...
    -- Same as above, except using an more than one units purchased to ensure unit cost calculation is correct
    -- Table driven tests would be GREAT here
    procedure test_single_use_supply_split_with_multiple_units_purchased is
    projectId int;
    projectId1 int;
    supplyId int;
    rowCount int;
    ...
    begin
        ...
        -- Act
        sus.bsa_proc_split_single_use_supply_among_projects('' || projectId || ':' || projectId1, 'single use supply 22222', 'item description', CURRENT_DATE, 'N', 5.00, 10, NULL);
        ...
        ut.expect(description).to_( equal('item description (Split Among Projects: Test Project 11111, Test Project 33333)') );
        -- 5.01 / 2 should round to 2.51
        ut.expect(unitCost).to_( equal(25.00) );
        -- Assert for the second project
        SELECT id, description, unitCost INTO supplyId, description, unitCost from BSA_SINGLE_USE_SUPPLY where name = 'single use supply 22222' and project_id = projectId1;
        ut.expect(supplyId).not_to( be_null() );
        ut.expect(description).to_( equal('item description (Split Among Projects: Test Project 11111, Test Project 33333)') );
